...
const RAINBOW_STYLES = [
  {
    ...
    light: [55, 68],
    wrap: true,
    // マゼンタから色相を丸ごと一周してマゼンタに戻る、彩度・明度を固定した純粋な虹。
    // 色相は昇順に回す(マゼンタ→赤→黄→緑→シアン→青→紫→マゼンタ)。
    // 他の6配色と違い明度を動かさないのは意図的な選択で、
    // 「ビビッド」の名のとおり色そのものの鮮やかさを優先している。
    //
    // トレードオフとして、最も遅い所と最も速い所が完全に同じ色になり、
    // 速度の読み取りは効かなくなる。速度を読みたい場合は他の配色を選ぶ。
    // (キャンディ・パステルが以前壊れていたのは色相が戻ること自体ではなく、
    //  明度まで揃っていて意図せず速度が読めなくなっていたため。こちらは承知のうえ)
    speedStops: [
      { u: 0.0, h: 315, s: 95, l: 58 }, // マゼンタ
      { u: 1 / 6, h: 15, s: 95, l: 58 }, // 朱
      { u: 2 / 6, h: 75, s: 95, l: 58 }, // 黄
      { u: 3 / 6, h: 135, s: 95, l: 58 }, // 緑
      { u: 4 / 6, h: 195, s: 95, l: 58 }, // シアン寄りの青
      { u: 5 / 6, h: 255, s: 95, l: 58 }, // 青紫
      { u: 1.0, h: 315, s: 95, l: 58 }, // → マゼンタへ戻る(一周)
    ],
  },
  {
    id: "gold",
