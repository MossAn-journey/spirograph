...
// 基本方針は明度を単調に上げること(遅い=暗く沈む → 速い=光る)。
// 色相だけに頼らないので、モノクロ印刷や色覚特性の差があっても速度差が読める。
// ただしビビッドだけは意図的にこの方針から外し、明度を固定して色相のみを
// 一周させている(詳細はビビッドのコメント参照)。
const RAINBOW_STYLES = [
  {
    ...
    light: [55, 68],
    wrap: true,
    // マゼンタから色相を昇順にほぼ一周し、紫で止める(彩度・明度は固定)。
    // 他の6配色と違い明度を動かさないのは意図的な選択で、
    // 「ビビッド」の名のとおり色そのものの鮮やかさを優先している。
    //
    // 一周を閉じずマゼンタ(315)の手前の紫(280)で終わらせているので、
    // 遅い端と速い端は 35 度離れた別の色になる。
    // 描いた図では外周がマゼンタ、中心が紫になる。
    // ただし明度が一定なため、速度の読み取り自体はこの配色では効かない。
    // 速度を読みたい場合は他の6配色を選ぶ。
    // (キャンディ・パステルが以前壊れていたのは色相が戻ること自体ではなく、
    //  明度まで揃っていて意図せず速度が読めなくなっていたため。こちらは承知のうえ)
    //
    // 先頭のアンカーを詰めているのは、等間隔だとマゼンタが 9% しか出ず
    // すぐ赤へ移ってしまい「外周がマゼンタ」に見えないため
    speedStops: [
      { u: 0.0, h: 315, s: 95, l: 58 }, // マゼンタ
      { u: 0.1, h: 342, s: 95, l: 58 }, // ピンク寄り(マゼンタを留める)
      { u: 0.25, h: 12, s: 95, l: 58 }, // 朱
      { u: 0.42, h: 62, s: 95, l: 58 }, // 黄
      { u: 0.58, h: 120, s: 95, l: 58 }, // 緑
      { u: 0.74, h: 178, s: 95, l: 58 }, // シアン
      { u: 0.88, h: 232, s: 95, l: 58 }, // 青
      { u: 1.0, h: 280, s: 95, l: 58 }, // 紫(一周は閉じない)
    ],
  },
  {
    id: "gold",
